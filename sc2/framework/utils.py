...
def sample_with_imitate(model, frozen_offline_model, critic_model, state, obs, sample=False, actions=None, rtgs=None,
           timesteps=None, available_actions=None):
    """
    take a conditioning sequence of indices in x (of shape (b,t)) and predict the next token in
    the sequence, feeding the predictions back into the model each time. Clearly the sampling
    ...
    of block_size, unlike an RNN that has an infinite context window.
    """
    # sample 这个参数是 Rolloout里面的是否train标志
    if torch.cuda.is_available():
        block_size = model.module.get_block_size()
    else:
        block_size = model.get_block_size()
    ...
    rtgs = rtgs if rtgs.size(1) <= block_size // 3 else rtgs[:, -block_size // 3:]  # crop context if needed
    timesteps = timesteps if timesteps.size(1) <= block_size // 3 else timesteps[:,
                                                                       -block_size // 3:]  # crop context if needed
    ...
    if sample:
        ...
        _, a = torch.topk(probs, k=1, dim=-1)
    # ##################### IDEA 2
    logits_frozen = frozen_offline_model(obs_cond, pre_actions=actions, rtgs=rtgs, timesteps=timesteps)
    # pluck the logits at the final step and scale by temperature
    logits_frozen = logits_frozen[:, -1, :]
    # apply softmax to convert to probabilities
    if available_actions is not None:
        logits_frozen[available_actions == 0] = -1e10
    probs_frozen = F.softmax(logits_frozen, dim=-1)
    if sample:
        a_frozen = torch.multinomial(probs_frozen, num_samples=1)
    else:
        _, a_frozen = torch.topk(probs_frozen, k=1, dim=-1)

    v = critic_model(state_cond, pre_actions=actions, rtgs=rtgs, timesteps=timesteps).detach()
    v = v[:, -1, :]

    return a, probs, a_frozen, probs_frozen, v
